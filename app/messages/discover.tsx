...
export default function DiscoverUsersScreen() {
  ...
  const runSearch = async (term: string) => {
    ...
    try {
      ...
      // Map database User to app User if needed
        ...
        ListEmptyComponent={
          !isSearching && (
            <View style={styles.emptyState}>
              <Text style={styles.emptyTitle}>Search for people to chat</Text>
              <Text style={styles.emptySubtitle}>Type a name to discover and connect</Text>
            </View>
          )
        }
      />
...
const styles = StyleSheet.create({
  ...
  searchInput: {
    flex: 1,
    color: Colors.white,
    // Remove the black focus outline on web
    outlineStyle: 'none',
    outlineWidth: 0,
